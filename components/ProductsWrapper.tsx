...
const ProductsWrapper: React.FC<PropType> = ({ categorySlug, categoryName }) => {
  ...
  useEffect(() => {
    const fetchProductsAndSpecs = async () => {
      ...
      try {
        ...
        console.log('Filtered Products:', fetchedProducts.products); // Log the filtered products
  ...
  }, [categorySlug, filters, currentPage]); // Re-fetch when categorySlug, filters, or currentPage changes

  // Handle filter changes from FilterSection component
  const handleFiltersChange = (updatedFilters: Record<string, string[]>) => {
    console.log('Selected Filters:', updatedFilters); // Log the updated filters
      ...
      <div className='w-full max-w-6xl mx-auto'>
        <div className='md:mx-4 my-10 mx-4 flex md:flex-row lg:flex-row items-center justify-between'>
          ...
          <form className='hidden lg:block md:block'>
            <select
              id='sort'
              className='bg-white border border-lightGray text-gray-900 text-sm rounded-lg focus:ring-lightGray focus:border-lightGray block w-full p-2.5'
            >
