...
const RenderProductSummary: React.FC<RenderProductSummaryProps> = ({ product }) => {
  ...
  useEffect(() => {
    // Update delivery range when the component mounts
    setDeliveryRange(calculateDeliveryRange());
  }, []);

  if (!product) return null; // Return null if product data is not available
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="flex flex-wrap -mx-4">
            <div className="w-full md:w-1/2 px-4 mb-8">
              {/* Main Product Image */}
              <img
                src={product.images[0]?.image || "placeholder-image-url"}
                alt="Product"
            ...
            <div className="w-full md:w-1/2 px-4">
              ...
              <div className="flex items-center mb-4">
                {/* Assume a static 5-star rating for now; update as needed */}
                {[...Array(5)].map((_, index) => (
                  <svg
                    key={index}
              ...
              {/* Description */}
              <p className="text-gray-700 mb-3 text-sm">{product.specifications.map(spec => `${spec.specification_name}: ${spec.value}`).join(', ')}</p>
